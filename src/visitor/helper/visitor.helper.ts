...
import type { IVisitorHelper, VisitorStatsResult, VisitorSummaryResult } from '../interface/visitor.helper.interface';
...
export class VisitorHelper implements IVisitorHelper {
  ...
  async getPublicSummary(): Promise<VisitorSummaryResult> {
    const total = await this.visitorModel.countDocuments().exec();
